...
STREAMLIT_LOCAL_PORT="${STREAMLIT_LOCAL_PORT:-8501}"
...
COMMANDS
  ...
  streamlit-tunnel        SSH tunnel for the Streamlit UI (local port \$STREAMLIT_LOCAL_PORT)
...
CONFIG
  ...
  STREAMLIT_LOCAL_PORT = $STREAMLIT_LOCAL_PORT

Override via environment variables WORKSHOP_INSTANCE_ID, WORKSHOP_KEY_PATH,
AWS_REGION, JUPYTER_LOCAL_PORT, STREAMLIT_LOCAL_PORT, or edit the top of this script.
EOF
}
...
cmd_streamlit_tunnel() {
    ...
    echo "Tunnel: localhost:${STREAMLIT_LOCAL_PORT} → ${ip}:8501"
    echo ""
    echo "Make sure the Streamlit app is running on the instance. In a separate"
    echo "SSH session there:"
    echo "  cd ~/protein-design-workshop"
    echo "  ./start_app.sh"
    echo ""
    echo "Then open this URL in your browser:"
    echo "  http://localhost:${STREAMLIT_LOCAL_PORT}"
    echo ""
    echo "Ctrl-C to close the tunnel."
    echo ""
    exec ssh -L "${STREAMLIT_LOCAL_PORT}:localhost:8501" -i "$KEY_PATH" "$SSH_USER@$ip"
}
...
case "${1:-}" in
    ...
    ssh)              cmd_ssh ;;
    tunnel)           cmd_tunnel ;;
    streamlit-tunnel) cmd_streamlit_tunnel ;;
    ip)               cmd_ip ;;
